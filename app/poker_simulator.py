...
class PokerSimulator:
    ...
    def __run_pre_flop_sim(self, n_runs) -> None:
        """Runs pre_flop simulation sequentially in chunks"""
        try:
            start_time = time()
            chunk_results = []
            ...
            for chunk_number in range(1, n_runs + 1, self.chunk_size):
                ...
                # Run simulations sequentially (no parallelism)
                for _ in range(chunk_number, chunk_number + current_chunk_size):
                    result = self._run_single_pre_flop_sim()
